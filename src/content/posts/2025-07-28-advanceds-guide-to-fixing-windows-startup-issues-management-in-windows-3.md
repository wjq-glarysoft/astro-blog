...
2\. Inspect and Edit Windows Registry - In Safe Mode or from recovery environment, run regedit. - Navigate to HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services to review drivers and services set to start at boot. Disable suspected problematic entries (always back up the registry first).
