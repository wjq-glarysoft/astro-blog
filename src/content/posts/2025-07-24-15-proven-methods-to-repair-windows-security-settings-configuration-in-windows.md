...
14\. Manually Repair the Registry Be cautious and back up the registry before making changes. Open Registry Editor (regedit.exe). Navigate to: HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows Defender Ensure settings like “DisableAntiSpyware” are set to 0. Likewise, check: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\wscsvc Confirm “Start” is set to 2 (Automatic).
