...
2\. Disable Cortana: - Open the registry editor by typing \`regedit\` in the Run dialog. - Navigate to \`HKEY_LOCAL_MACHINE\\SOFTWARE\\Policies\\Microsoft\\Windows\\Windows Search\`. - Right-click in the right pane, select 'New', then 'DWORD (32-bit) Value', and name it 'AllowCortana'. - Set its value to 0 to disable Cortana.
