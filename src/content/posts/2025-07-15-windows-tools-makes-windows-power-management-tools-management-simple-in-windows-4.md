...
1\. Open Task Scheduler from the Start menu. 2. Create a new task and set the trigger (such as a specific time). 3. In Actions, use the command line tool powercfg to set the desired plan: Example: powercfg /setactive SCHEME_GUID (replace SCHEME_GUID with the GUID of your power plan, found using powercfg /list).
