...
\- Open Command Prompt as an administrator. - To export a power plan, type: \`powercfg -export \`. For example: \`powercfg -export "C:\\backup\\Balanced.pow" SCHEME_BALANCED\`. - Note: To find the GUID of your power plan, use the command \`powercfg /list\`.
