...
powercfg /list Displays all available power schemes. powercfg /setactive SCHEME_GUID Switches instantly to the desired scheme. powercfg /hibernate on|off Enables or disables hibernation.

To create a custom, highly-tuned plan, duplicate the High Performance plan with:

powercfg /duplication SCHEME_GUID
...
Use powercfg /devicequery wake_armed to list devices allowed to wake your PC. Disable unwanted wake events in Device Manager: locate the device, open Properties, go to the Power Management tab, and uncheck "Allow this device to wake the computer." This is invaluable for stopping network adapters or mice from waking your machine inadvertently.
