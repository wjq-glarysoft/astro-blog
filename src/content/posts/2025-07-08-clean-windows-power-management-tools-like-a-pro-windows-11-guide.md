...
Identify which devices wake your PC: powercfg /devicequery wake_armed
...
Duplicate and modify an existing plan: powercfg /duplicate scheme_GUID

Delete obsolete or unused plans: powercfg /delete scheme_GUID
...
1\. Open Task Scheduler (search in Start). 2. Create a new task (not basic). 3. Under Triggers, set a schedule (e.g., at 2 AM every weekday). 4. Under Actions, use: - powercfg /hibernate on - rundll32.exe powrprof.dll,SetSuspendState 0,1,0 - Or scripts to switch plans: powercfg /setactive scheme_GUID
